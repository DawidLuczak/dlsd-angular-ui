...
export const DROPDOWN_CONFIG = <T>() =>
  new InjectionToken<DropdownConfig<T>>('Options to display in a dropdown');
...
export class DropdownComponent<T> implements OnInit, AfterViewInit {
  ...
  protected focusInputWithArrows(): void {
    ...
    if (index === this.focusedOptionIndex()) {
      if (!this.config.addOption && !this.config.search) {
        this.focusOption(index === 0 ? -1 : 0);
      } else {
        this.config.focusSourceElement?.();
      }
    } else {
